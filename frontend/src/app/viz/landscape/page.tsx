...
export default function EfficiencyLandscapePage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-2">Predicted Efficiency Landscape</h1>
        <p className="text-gray-600">
          Offensive vs Defensive rating with bands for Final Four and Title Favorites
        </p>
      </div>
      ...
      <div 
        ...
        role="img"
        aria-label="Efficiency Landscape scatter plot showing Offensive Rating vs Defensive Rating with tier boundaries"
      >
        {loading && (
          ...
          <>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <p className="text-xs text-gray-500 text-center">
                Showing {data.teams.length} teams | Season {data.season_display} | Max Net: {data.max_net}
              </p>
            </div>
          </>
        )}
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mt-6">
        ...
        <p className="mb-4">
          The <strong>Efficiency Landscape</strong> maps teams by their offensive and defensive efficiency ratings. 
          Diagonal tier boundaries separate championship contenders from the rest of the field.
        </p>
        
        <h3 className="text-xl font-bold mb-3">How It Works</h3>
        <ul className="list-disc list-inside space-y-2 mb-4">
          <li><strong>X-axis:</strong> D-Rate (Defensive Rating - lower is better)</li>
          <li><strong>Y-axis:</strong> O-Rate (Offensive Rating - higher is better)</li>
          <li><strong>Net Rating:</strong> O-Rate + D-Rate (sum determines overall strength)</li>
          <li><strong>Title Favorites:</strong> Net ≥ Max Net - 6 (top contenders)</li>
          <li><strong>Final Four Potential:</strong> Net ≥ Max Net - 12 (strong teams)</li>
          <li><strong>Hit or Miss:</strong> Net ≥ Max Net - 18 (bubble teams)</li>
          <li><strong>The Rest:</strong> Teams below Hit or Miss threshold</li>
        </ul>
        
        <p className="text-sm text-gray-600">
          Diagonal lines represent constant net ratings. Teams in the upper-left corner (great offense, great defense) 
...
function EfficiencyLandscapeChart({ data, hoveredTeam, onTeamHover, onTeamClick }: EfficiencyLandscapeChartProps) {
  ...
  const { xScale, yScale, tierLines, xTicks, yTicks } = useMemo(() => {
    ...
    const xScale = (d: number) => {
      return margin.left + ((d - (minD - dPadding)) / ((maxD + dPadding) - (minD - dPadding))) * plotWidth;
    };
    ...
    // Compute max sum (O-Rate + D-Rate) for diagonal tier lines with slope -1
    const maxSum = Math.max(...teams.map(t => t.o_rate + t.d_rate));
    
    // Tier cutoffs based on sum values (for diagonal lines: o + d = constant)
    const titleFavoritesSum = maxSum - defaults.title_delta;
    const final4Sum = maxSum - defaults.final4_delta;
    const hitMissSum = maxSum - defaults.hit_miss_delta;
    
    // Compute diagonal lines (O-Rate + D-Rate = constant, so O-Rate = -D-Rate + sum)
    // For each sum value, find line segment clipped to plot bounds
    const computeDiagonalLine = (sumValue: number) => {
      // Line equation: o = -d + sumValue (slope -1)
    ...
    const tierLines = [
      { sum: titleFavoritesSum, label: 'Title Favorites', color: '#ED713A', width: 3, dash: 'none' },
      { sum: final4Sum, label: 'Final 4', color: '#30A2DA', width: 2, dash: '5,5' },
      { sum: hitMissSum, label: 'Hit or Miss', color: '#999', width: 2, dash: '5,5' },
    ].map(tier => ({
      ...tier,
      points: computeDiagonalLine(tier.sum),
    }));
    ...
    <div className="relative">
      ...
      {tooltipTeam && (
        <div className="absolute top-4 left-4 bg-white border border-gray-300 rounded-lg shadow-lg p-4 max-w-xs z-10">
          <div className="font-bold text-lg mb-1">{tooltipTeam.team_name}</div>
