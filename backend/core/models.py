...
class TeamExternalId(models.Model):
    ...
    external_id = models.CharField(max_length=100, help_text="External source's team ID")
    external_name = models.CharField(max_length=200, help_text="External source's team name")
    confidence = models.FloatField(
        default=1.0,
        help_text="Match confidence score (0.0-1.0, 1.0 = exact match)"
    )
    is_manual_override = models.BooleanField(
        default=False,
        help_text="True if this mapping was manually specified"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Game(models.Model):
    ...
    season_year = models.IntegerField(
        db_index=True,
        help_text="Ending year (2026 for 2025-26 season)"
    )
    ...
    # Data source
    source = models.CharField(max_length=20, default='ncaa', help_text="Data source (ncaa, espn)")
...
class TeamGameStats(models.Model):
    ...
    minutes = models.IntegerField(null=True, blank=True, help_text="Total minutes played")
    ...
    def _get_opp_stats(self):
        ...
        if not hasattr(self, '_cached_opp_stats'):
            self._cached_opp_stats = self.game.team_stats.exclude(team=self.team).first()
...
class ScoringEvent(models.Model):
    ...
    """
    game = models.ForeignKey(Game, on_delete=models.CASCADE, related_name='scoring_events')
    ...
    clock = models.CharField(
        max_length=10,
        help_text="Game clock (e.g., '15:32', '0:45')"
    )
    ...
    points = models.IntegerField(
        help_text="Points scored on this event (1, 2, or 3)"
    )
    ...
    def __str__(self):
        return f"Game {self.game.id} #{self.seq}: {self.scoring_team.name} +{self.points}"
...
class TeamSeasonRatings(models.Model):
    ...
    """
    team = models.ForeignKey(Team, on_delete=models.CASCADE, related_name='season_ratings')
    season = models.ForeignKey(Season, on_delete=models.CASCADE, related_name='team_ratings')
    ...
    adj_orb_pct = models.FloatField(default=0.0, help_text="Adjusted Offensive Rebound %")
    ...
    # Model parameters
    games_played = models.IntegerField(default=0, help_text="Total games played (all opponents)")
    ...
    d1_games_played = models.IntegerField(default=0, help_text="Games vs D1 opponents only")
    total_possessions = models.FloatField(default=0.0)
    hca_estimate = models.FloatField(
        null=True,
        blank=True,
        help_text="Home court advantage (in points per 100 poss)"
    )
...
class TeamSeasonMetrics(models.Model):
    ...
    """
    team = models.ForeignKey(Team, on_delete=models.CASCADE, related_name='season_metrics')
    season = models.ForeignKey(Season, on_delete=models.CASCADE, related_name='team_metrics')
...
class NationalAverages(models.Model):
    ...
    """
    season = models.OneToOneField(Season, on_delete=models.CASCADE, related_name='national_averages')
