...
class NCAAAPIClient:
    ...
    def __init__(
        ...
        retry_strategy = Retry(
            ...
            status_forcelist=[429, 500, 503, 504],  # Removed 502 - too many when API is overloaded
            allowed_methods=["GET"]
        )
    ...
    def get_game_details(
        ...
        for team_box in stats_data.get('teamBoxscore', []):
            ...
            normalized_stats = {
                ...
                'fieldGoalsAttempted': self._parse_int(team_stats.get('fieldGoalsAttempted')),
                'threePointFieldGoalsMade': self._parse_int(team_stats.get('threePointsMade')),
                'threePointFieldGoalsAttempted': self._parse_int(team_stats.get('threePointsAttempted')),
                'freeThrowsMade': self._parse_int(team_stats.get('freeThrowsMade')),
                'freeThrowsAttempted': self._parse_int(team_stats.get('freeThrowsAttempted')),
                'offensiveRebounds': self._parse_int(team_stats.get('offensiveRebounds')),
                'defensiveRebounds': self._parse_int(team_stats.get('totalRebounds', 0)) - self._parse_int(team_stats.get('offensiveRebounds', 0)),
            ...
            points = (
                ...
                normalized_stats['threePointFieldGoalsMade'] +  # Already counted in FGM, so add the extra point
            ...
            is_home = next((t.get('isHome') for t in teams if str(t.get('teamId')) == team_id), None)
...
class ESPNAPIClient:
    ...
    def _normalize_game_details(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if competitions:
            ...
            result['status'] = comp.get('status', {}).get('type', {}).get('state', 'pre')
            ...
            for team_data in teams:
                ...
                is_home = home_team and str(home_team.get('team', {}).get('id')) == str(team_id)
                ...
                for stat in team_data.get('statistics', []):
                    ...
                    if stat_name in stat_map:
                        ...
                        if '-' in str(stat_val) and 'pct' not in stat_name:
                            ...
                            base_key = stat_map[stat_name].replace('Made', '').replace('Made', '')
                            stats[f"{base_key}Made"] = int(made) if made.isdigit() else 0
                            stats[f"{base_key}Attempted"] = int(attempted) if attempted.isdigit() else 0
                        else:
                            try:
                                stats[stat_map[stat_name]] = int(stat_val) if stat_val.replace('-', '').isdigit() else 0
                            except:
                                pass
