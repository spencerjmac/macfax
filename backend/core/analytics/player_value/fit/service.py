...
For sandbox / hypothetical rosters call the engine directly:
    from core.analytics.player_value.fit.engine import (
...
from core.analytics.player_value.fit.archetypes import PlayerFitInput, SeasonRefs
from core.analytics.player_value.fit.engine import score_roster_fit
from core.analytics.player_value.fit.constants import SEASON_REF_MIN_PLAYERS
...
def _build_season_refs(stats_list: list[dict]) -> SeasonRefs:
    ...
    if len(qual) < SEASON_REF_MIN_PLAYERS:
        from core.analytics.player_value.fit.archetypes import DEFAULT_REFS
        return DEFAULT_REFS
