...
from core.models import (
    Season, Team, Game, TeamGameStats, ScoringEvent, TeamSeasonMetrics
)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--teams',
            nargs='+',
            help='Specific team slugs to process (default: all)'
        )
    ...
    def handle(self, *args, **options):
        ...
        for team in teams:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"  ERROR - {team.name}: {e}")
                )
    ...
    def _compute_derived_metrics(
        ...
        drtg = 100 * (totals['total_pts_allowed'] / total_opp_poss) if total_opp_poss > 0 else 0
        net_rtg = ortg - drtg
        ...
        opp_orb_pct = (opp_oreb / (opp_oreb + dreb) * 100) if (opp_oreb + dreb) > 0 else 0
