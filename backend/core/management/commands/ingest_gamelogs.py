...
from core.models import (
    Season, Team, Game, TeamGameStats, ScoringEvent, TeamExternalId
)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--refresh',
            action='store_true',
            help='Force refresh existing games'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def _build_team_mappings(
        ...
        self.stdout.write(
            self.style.SUCCESS(f"  OK - {existing_count} {source.upper()} team mappings loaded")
        )
        ...
        while current_date <= end_date:
            ...
            try:
                ...
                for i, game_data in enumerate(games, 1):
                    game_id = str(game_data.get('id') or game_data.get('gameID', 'unknown'))
                    try:
                        ...
                        if result['created']:
                            ...
                            stats['scoring_events_created'] += result['scoring_events']
                            self.stdout.write(f"    ✓ [{i}/{len(games)}] Created game {game_id}")
                        elif result['updated']:
                            stats['games_updated'] += 1
                            self.stdout.write(f"    ↻ [{i}/{len(games)}] Updated game {game_id}")
                        else:
                        ...
                        self.stdout.write(
                            self.style.ERROR(f"    ✗ [{i}/{len(games)}] Error processing game {game_id}: {e}")
                        )
    ...
    def _process_game(
        ...
        home_name = home_team_data.get('name') or home_team_data.get('team', {}).get('displayName')
        away_name = away_team_data.get('name') or away_team_data.get('team', {}).get('displayName')
        home_id = str(home_team_data.get('id') or home_team_data.get('team', {}).get('id', ''))
        away_id = str(away_team_data.get('id') or away_team_data.get('team', {}).get('id', ''))
        ...
        if dry_run:
            ...
            return {'created': True, 'updated': False, 'team_stats': 2, 'scoring_events': 0}
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f"      ⏱ Timeout for game {game_id}, using scoreboard data only")
            )
            full_game_data = game_data
            ...
            self.stdout.write(
                self.style.WARNING(f"      ⚠ Network error for game {game_id}, using scoreboard data only")
            )
            full_game_data = game_data
        except NCAAAPIError as e:
            ...
            self.stdout.write(
                self.style.WARNING(f"      ⚠ Data unavailable for game {game_id}, using scoreboard data only")
            )
            full_game_data = game_data
        except Exception as e:
    ...
    def _map_team(
        ...
        if team_name and any(indicator.lower() in team_name.lower() for indicator in non_d1_indicators):
            if not dry_run:
    ...
    def _upsert_game(
        ...
        game_date_str = game_data.get('date') or game_data.get('gameDate') or game_data.get('startDate')
        if game_date_str:
            try:
        ...
        neutral = game_data.get('neutralSite', False) or game_data.get('neutral_site', False)
    ...
    def _extract_team_stats(self, game: Game, game_data: Dict) -> int:
        ...
        for team_data, team, opponent, location in [
            (home_stats, game.home_team, game.away_team, 'H' if not game.neutral_site else 'N'),
            (away_stats, game.away_team, game.home_team, 'A' if not game.neutral_site else 'N'),
        ]:
            if not team_data:
                continue
