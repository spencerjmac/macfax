...
Runs the complete NCAA data pipeline (team + player statistics and rankings).
...
class Command(BaseCommand):
    help = "Run the complete NCAA CBB Analytics data pipeline"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("\n" + "=" * 80)
        self.stdout.write("MACFAX — NCAA DATA PIPELINE UPDATE")
        ...
        # HCA and sigma prereq for compute_sor, compute_game_value, compute_wab
        self._run_step(
    ...
    def _print_summary(self, start_time, end_time, duration, steps, failed):
        ...
        if not failed:
            self.stdout.write(self.style.SUCCESS("All NCAA data updated successfully!"))
