...
class PostTournamentManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(is_pre_tournament=False)
...
class TeamSeasonRatings(models.Model):
    ...
    is_pre_tournament = models.BooleanField(
        default=False,
        help_text="True if this rating was computed using only pre-tournament games (on or before Selection Sunday)",
    )
...
class TeamSeasonMetrics(models.Model):
    ...
    is_pre_tournament = models.BooleanField(
        default=False,
        help_text="True if this metric was computed using only pre-tournament games (on or before Selection Sunday)",
    )
