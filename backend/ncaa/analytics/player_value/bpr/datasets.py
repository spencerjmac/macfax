...
from ncaa.analytics.player_value.bpr.constants import FTA_POSS_FACTOR, MIN_SEGMENT_POSS
...
def build_rapm_dataset(season_years: "int | list[int]", verbose: bool = True) -> dict:
    ...
    for yr in sorted(years):
        ...
        if total == 0:
            if yr == target_year:
                # Always include the target year so the pipeline can fail with a
                # meaningful error rather than "max() arg is an empty sequence".
                valid_years.append(yr)
                if verbose:
                    logger.warning(
                        f"BPR dataset: TARGET year {yr} has no stints yet — "
                        f"BPR cannot be computed (run compute_pbp_stints first)"
                    )
            else:
                if verbose:
