...
from core.models import (
    ...
    Game, TeamGameStats, ScoringEvent, TeamSeasonMetrics, TeamSeasonRatings
)
...
class RankingsSerializer(serializers.Serializer):
    ...
    def get_conference(self, obj):
        ...
        try:
            metrics = TeamSeasonMetrics.objects.filter(team=obj.team, season=obj.season).first()
        ...
        conf_map = {
            ...
            'michigan': 'B10', 'michigan-state': 'B10', 'michigan-st': 'B10', 'ohio-state': 'B10', 'ohio-st': 'B10', 'purdue': 'B10',
            'illinois': 'B10', 'indiana': 'B10', 'iowa': 'B10', 'wisconsin': 'B10',
            'maryland': 'B10', 'penn-state': 'B10', 'penn-st': 'B10', 'rutgers': 'B10', 'nebraska': 'B10',
            'minnesota': 'B10', 'northwestern': 'B10',
            'ucla': 'B10', 'usc': 'B10', 'oregon': 'B10', 'washington': 'B10',
            
            # ACC - Atlantic Coast Conference
            'duke': 'ACC', 'north-carolina': 'ACC', 'unc': 'ACC', 'virginia': 'ACC', 'louisville': 'ACC',
            'syracuse': 'ACC', 'florida-state': 'ACC', 'florida-st': 'ACC', 'nc-state': 'ACC', 'clemson': 'ACC',
            'wake-forest': 'ACC', 'miami': 'ACC', 'miami-fl': 'ACC', 'miami-(fl)': 'ACC', 'georgia-tech': 'ACC', 'boston-college': 'ACC',
            'virginia-tech': 'ACC', 'pitt': 'ACC', 'pittsburgh': 'ACC', 'notre-dame': 'ACC',
            'california': 'ACC', 'stanford': 'ACC', 'smu': 'ACC',
            
            # Big 12 (B12)
            'kansas': 'B12', 'baylor': 'B12', 'iowa-st': 'B12', 'iowa-state': 'B12',
            'texas-tech': 'B12', 'tcu': 'B12', 'oklahoma-state': 'B12', 'oklahoma-st': 'B12',
            'west-virginia': 'B12', 'kansas-state': 'B12', 'kansas-st': 'B12',
            'houston': 'B12', 'cincinnati': 'B12', 'ucf': 'B12', 'byu': 'B12',
            'colorado': 'B12', 'arizona': 'B12', 'arizona-state': 'B12', 'arizona-st': 'B12', 'utah': 'B12',
            
            # SEC - Southeastern Conference
            'kentucky': 'SEC', 'florida': 'SEC', 'tennessee': 'SEC', 'arkansas': 'SEC',
            'alabama': 'SEC', 'auburn': 'SEC', 'lsu': 'SEC', 'mississippi-state': 'SEC', 'mississippi-st': 'SEC',
            'georgia': 'SEC', 'missouri': 'SEC', 'south-carolina': 'SEC', 'vanderbilt': 'SEC',
            'texas': 'SEC', 'texas-am': 'SEC', 'texas-a-m': 'SEC', 'texas-a&m': 'SEC', 'ole-miss': 'SEC', 'mississippi': 'SEC',
            'oklahoma': 'SEC',
            
            # Big East (BE)
            'connecticut': 'BE', 'uconn': 'BE', 'villanova': 'BE', 'creighton': 'BE', 'marquette': 'BE',
            'st-johns': 'BE', 'st-john-s': 'BE', "st-john's": 'BE', 'providence': 'BE', 'xavier': 'BE', 'butler': 'BE',
            'seton-hall': 'BE', 'depaul': 'BE', 'georgetown': 'BE',
            
            # WCC - West Coast Conference
            'gonzaga': 'WCC', 'saint-marys': 'WCC', 'saint-mary-s': 'WCC', "saint-mary's": 'WCC', 'st-marys': 'WCC',
            'san-francisco': 'WCC', 'santa-clara': 'WCC', 'loyola-marymount': 'WCC',
            'pepperdine': 'WCC', 'portland': 'WCC', 'pacific': 'WCC', 'san-diego': 'WCC',
            'oregon-st': 'WCC', 'washington-st': 'WCC',
            'seattle': 'WCC', 'seattle-u': 'WCC',
            
            # Mountain West (MWC)
            'utah-st': 'MWC', 'utah-state': 'MWC', 'san-diego-state': 'MWC', 'san-diego-st': 'MWC', 'sdsu': 'MWC',
            'nevada': 'MWC', 'unlv': 'MWC', 'colorado-state': 'MWC', 'colorado-st': 'MWC',
            'new-mexico': 'MWC', 'boise-state': 'MWC', 'boise-st': 'MWC', 'wyoming': 'MWC',
            'fresno-state': 'MWC', 'fresno-st': 'MWC', 'air-force': 'MWC', 'san-jose-state': 'MWC', 'san-jose-st': 'MWC',
            'grand-canyon': 'MWC',
            
            # Atlantic 10 (A10)
            'saint-louis': 'A10', 'st-louis': 'A10', 'dayton': 'A10', 'vcu': 'A10', 'richmond': 'A10',
            'st-bonaventure': 'A10', 'saint-bonaventure': 'A10', 'davidson': 'A10', 'rhode-island': 'A10',
            'george-mason': 'A10', 'fordham': 'A10',
            'st-josephs': 'A10', 'saint-josephs': 'A10', "saint-joseph's": 'A10', 'la-salle': 'A10', 'duquesne': 'A10', 'george-washington': 'A10', 'loyola-chicago': 'A10',
            
            # American Athletic Conference (Amer)
            'memphis': 'Amer', 'temple': 'Amer', 'wichita-state': 'Amer', 'wichita-st': 'Amer',
            'east-carolina': 'Amer', 'south-florida': 'Amer', 'tulsa': 'Amer', 'tulane': 'Amer',
            'north-texas': 'Amer', 'uab': 'Amer', 'charlotte': 'Amer', 'florida-atlantic': 'Amer', 'utsa': 'Amer', 'rice': 'Amer',
            
            # Conference USA (CUSA)
            'louisiana-tech': 'CUSA', 'western-kentucky': 'CUSA', 'middle-tennessee': 'CUSA',
            'jacksonville-state': 'CUSA', 'jacksonville-st': 'CUSA', 
            'kennesaw-state': 'CUSA', 'kennesaw-st': 'CUSA', 'liberty': 'CUSA',
            'new-mexico-state': 'CUSA', 'new-mexico-st': 'CUSA', 'sam-houston-state': 'CUSA', 'sam-houston-st': 'CUSA',
            'fiu': 'CUSA', 'utep': 'CUSA',
            
            # Missouri Valley (MVC)
            'illinois-state': 'MVC', 'illinois-st': 'MVC', 'indiana-state': 'MVC', 'indiana-st': 'MVC',
            'drake': 'MVC', 'bradley': 'MVC', 'northern-iowa': 'MVC', 'southern-illinois': 'MVC',
            'missouri-state': 'MVC', 'missouri-st': 'MVC', 'valparaiso': 'MVC', 'belmont': 'MVC', 'murray-state': 'MVC', 'murray-st': 'MVC',
            'evansville': 'MVC', 'uic': 'MVC', 'illinois-chicago': 'MVC',
            
            # Big West (BW)
            'hawaii': 'BW', 'uc-irvine': 'BW', 'uc-davis': 'BW', 'uc-santa-barbara': 'BW', 'ucsb': 'BW',
            'cal-poly': 'BW', 'cal-state-northridge': 'BW', 'cal-st-northridge': 'BW', 'csun': 'BW', 'cal-state-fullerton': 'BW', 'cal-st-fullerton': 'BW',
            'uc-riverside': 'BW', 'cal-state-bakersfield': 'BW', 'cal-st-bakersfield': 'BW', 'long-beach-state': 'BW', 'long-beach-st': 'BW',
            'uc-san-diego': 'BW',
            
            # Summit League (Sum)
            'south-dakota-state': 'Sum', 'south-dakota-st': 'Sum', 'sdsu': 'Sum', 'north-dakota-state': 'Sum', 'north-dakota-st': 'Sum',
            'oral-roberts': 'Sum', 'south-dakota': 'Sum', 'north-dakota': 'Sum', 'denver': 'Sum',
            'omaha': 'Sum', 'nebraska-omaha': 'Sum', 'kansas-city': 'Sum', 'umkc': 'Sum', 'st-thomas': 'Sum',
            
            # WAC - Western Athletic Conference
            'utah-valley': 'WAC', 'stephen-f-austin': 'WAC',
            'abilene-christian': 'WAC', 'southern-utah': 'WAC', 'ut-arlington': 'WAC', 'tarleton-state': 'WAC', 'tarleton-st': 'WAC',
            'cal-baptist': 'WAC', 'california-baptist': 'WAC', 'utah-tech': 'WAC',
            
            # Horizon League (Horz)
            'wright-state': 'Horz', 'wright-st': 'Horz', 'northern-kentucky': 'Horz', 'cleveland-state': 'Horz', 'cleveland-st': 'Horz',
            'oakland': 'Horz', 'youngstown-state': 'Horz', 'youngstown-st': 'Horz', 'milwaukee': 'Horz',
            'robert-morris': 'Horz', 'iupui': 'Horz', 'iu-indy': 'Horz', 'purdue-fort-wayne': 'Horz', 'green-bay': 'Horz',
            'detroit-mercy': 'Horz',
            
            # MAAC - Metro Atlantic Athletic Conference
            'iona': 'MAAC', 'marist': 'MAAC', 'manhattan': 'MAAC', 'quinnipiac': 'MAAC', 'fairfield': 'MAAC',
            'rider': 'MAAC', 'siena': 'MAAC', 'niagara': 'MAAC', 'canisius': 'MAAC',
            'mount-st-marys': 'MAAC', 'mount-st-mary-s': 'MAAC', "mount-st-mary's": 'MAAC', 'sacred-heart': 'MAAC', 'saint-peters': 'MAAC', 'saint-peter-s': 'MAAC', "saint-peter's": 'MAAC', 'st-peters': 'MAAC',
            
            # MAC - Mid-American Conference
            'toledo': 'MAC', 'akron': 'MAC', 'bowling-green': 'MAC', 'kent-state': 'MAC', 'kent-st': 'MAC',
            'miami-oh': 'MAC', 'miami-(oh)': 'MAC', 'ohio': 'MAC', 'ball-state': 'MAC', 'ball-st': 'MAC', 'buffalo': 'MAC',
            'central-michigan': 'MAC', 'eastern-michigan': 'MAC', 'massachusetts': 'MAC', 'umass': 'MAC', 'northern-illinois': 'MAC', 'western-michigan': 'MAC',
            
            # NEC - Northeast Conference
            'merrimack': 'NEC', 'central-connecticut': 'NEC', 'central-connecticut-st': 'NEC', 'liu': 'NEC',
            'wagner': 'NEC', 'stonehill': 'NEC', 'le-moyne': 'NEC', 'fairleigh-dickinson': 'NEC',
            'chicago-state': 'NEC', 'chicago-st': 'NEC', 'saint-francis': 'NEC', 'mercyhurst': 'NEC',
            
            # Big Sky (BSky)
            'montana': 'BSky', 'montana-state': 'BSky', 'montana-st': 'BSky', 'weber-state': 'BSky', 'weber-st': 'BSky',
            'idaho': 'BSky', 'idaho-state': 'BSky', 'idaho-st': 'BSky', 'northern-colorado': 'BSky',
            'portland-state': 'BSky', 'portland-st': 'BSky', 'eastern-washington': 'BSky', 'northern-arizona': 'BSky',
            'sacramento-state': 'BSky', 'sacramento-st': 'BSky',
            
            # OVC - Ohio Valley Conference
            'morehead-state': 'OVC', 'morehead-st': 'OVC', 'tennessee-state': 'OVC', 'tennessee-st': 'OVC',
            'ut-martin': 'OVC', 'tennessee-martin': 'OVC', 'tennessee-tech': 'OVC', 'lindenwood': 'OVC',
            'siu-edwardsville': 'OVC', 'little-rock': 'OVC', 'western-illinois': 'OVC',
            'eastern-illinois': 'OVC', 'southeast-missouri-st': 'OVC', 'southern-indiana': 'OVC',
            
            # CAA - Colonial Athletic Association
            'charleston': 'CAA', 'hofstra': 'CAA', 'drexel': 'CAA', 'delaware': 'CAA', 'towson': 'CAA',
            'unc-wilmington': 'CAA', 'monmouth': 'CAA', 'stony-brook': 'CAA', 'elon': 'CAA',
            'william-mary': 'CAA', 'william-&-mary': 'CAA', 'hampton': 'CAA', 'campbell': 'CAA', 'north-carolina-at': 'CAA', 'north-carolina-a&t': 'CAA', 'nc-at': 'CAA', 'nc-a&t': 'CAA',
            'northeastern': 'CAA',
            
            # Southern Conference (SC)
            'furman': 'SC', 'wofford': 'SC', 'unc-greensboro': 'SC', 'samford': 'SC',
            'east-tennessee-state': 'SC', 'east-tennessee-st': 'SC', 'etsu': 'SC', 'chattanooga': 'SC',
            'the-citadel': 'SC', 'citadel': 'SC', 'vmi': 'SC', 'western-carolina': 'SC', 'mercer': 'SC',
            
            # Sun Belt (SB)
            'james-madison': 'SB', 'coastal-carolina': 'SB', 'troy': 'SB', 'georgia-state': 'SB', 'georgia-st': 'SB',
            'georgia-southern': 'SB', 'app-state': 'SB', 'appalachian-state': 'SB', 'appalachian-st': 'SB',
            'louisiana': 'SB', 'marshall': 'SB', 'old-dominion': 'SB', 'southern-miss': 'SB', 'southern-mississippi': 'SB',
            'arkansas-state': 'SB', 'arkansas-st': 'SB', 'ulm': 'SB', 'louisiana-monroe': 'SB', 'texas-state': 'SB', 'texas-st': 'SB',
            'south-alabama': 'SB',
            
            # Southland (Slnd)
            'mcneese-state': 'Slnd', 'mcneese-st': 'Slnd', 'mcneese': 'Slnd', 'nicholls-state': 'Slnd', 'nicholls-st': 'Slnd', 'nicholls': 'Slnd',
            'northwestern-state': 'Slnd', 'northwestern-st': 'Slnd', 'southeastern-louisiana': 'Slnd',
            'texas-am-commerce': 'Slnd', 'east-texas-a&m': 'Slnd', 'houston-christian': 'Slnd', 'incarnate-word': 'Slnd', 'lamar': 'Slnd',
            'new-orleans': 'Slnd', 'texas-a&m-corpus-chris': 'Slnd', 'ut-rio-grande-valley': 'Slnd',
            
            # ASUN (ASun)
            'lipscomb': 'ASun', 'jacksonville': 'ASun', 'fgcu': 'ASun', 'florida-gulf-coast': 'ASun',
            'stetson': 'ASun', 'north-florida': 'ASun',
            'queens': 'ASun', 'eastern-kentucky': 'ASun', 'austin-peay': 'ASun', 'central-arkansas': 'ASun',
            'bellarmine': 'ASun', 'north-alabama': 'ASun', 'west-georgia': 'ASun',
            
            # America East (AE)
            'vermont': 'AE', 'bryant': 'AE', 'umbc': 'AE', 'umass-lowell': 'AE',
            'binghamton': 'AE', 'new-hampshire': 'AE', 'unh': 'AE', 'maine': 'AE', 'albany': 'AE', 'njit': 'AE',
            
            # Ivy League (Ivy)
            'princeton': 'Ivy', 'yale': 'Ivy', 'harvard': 'Ivy', 'cornell': 'Ivy',
            'penn': 'Ivy', 'pennsylvania': 'Ivy', 'brown': 'Ivy', 'columbia': 'Ivy', 'dartmouth': 'Ivy',
            
            # Patriot League (Pat)
            'colgate': 'Pat', 'army': 'Pat', 'navy': 'Pat', 'bucknell': 'Pat', 'holy-cross': 'Pat',
            'lehigh': 'Pat', 'lafayette': 'Pat', 'boston-university': 'Pat', 'american': 'Pat', 'loyola-maryland': 'Pat', 'loyola-md': 'Pat',
            
            # MEAC - Mid-Eastern Athletic Conference
            'norfolk-state': 'MEAC', 'norfolk-st': 'MEAC', 'howard': 'MEAC', 'north-carolina-central': 'MEAC', 'nc-central': 'MEAC',
            'morgan-state': 'MEAC', 'morgan-st': 'MEAC', 'south-carolina-state': 'MEAC', 'south-carolina-st': 'MEAC',
            'coppin-state': 'MEAC', 'coppin-st': 'MEAC', 'delaware-state': 'MEAC', 'delaware-st': 'MEAC',
            'maryland-eastern-shore': 'MEAC', 'umes': 'MEAC',
            
            # SWAC - Southwestern Athletic Conference
            'alabama-state': 'SWAC', 'alabama-st': 'SWAC', 'alabama-a&m': 'SWAC', 'alcorn-state': 'SWAC', 'alcorn-st': 'SWAC',
            'arkansas-pine-bluff': 'SWAC', 'bethune-cookman': 'SWAC', 'florida-am': 'SWAC', 'florida-a&m': 'SWAC', 'famu': 'SWAC',
            'grambling-state': 'SWAC', 'grambling-st': 'SWAC', 'jackson-state': 'SWAC', 'jackson-st': 'SWAC',
            'mississippi-valley-state': 'SWAC', 'mississippi-valley-st': 'SWAC', 'prairie-view-am': 'SWAC', 'prairie-view-a&m': 'SWAC',
            'southern': 'SWAC', 'texas-southern': 'SWAC',
            
            # Big South (BSth)
            'high-point': 'BSth', 'radford': 'BSth', 'longwood': 'BSth', 'gardner-webb': 'BSth',
            'winthrop': 'BSth', 'charleston-southern': 'BSth', 'presbyterian': 'BSth', 'unc-asheville': 'BSth',
            'usc-upstate': 'BSth',
        }
...
class GameSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Game
        fields = [
            'id', 'source_game_id', 'season_year', 'game_date', 'start_time_utc',
            'home_team_name', 'away_team_name', 'home_team_slug', 'away_team_slug',
            'home_score', 'away_score', 'status', 'neutral_site',
            'venue_name', 'venue_city', 'venue_state',
            'period_count', 'went_to_ot', 'winner_name',
...
class TeamGameStatsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TeamGameStats
        fields = [
            'id', 'team_name', 'team_slug', 'opponent_name', 'opponent_slug',
            'game_date', 'game_status', 'home_away',
            'pts', 'fgm', 'fga', 'fg3m', 'fg3a', 'ftm', 'fta',
            'fg2m', 'fg2a',  # Computed
            'fg_pct', 'fg3_pct', 'ft_pct',  # Computed percentages
...
class GameLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            # Shooting percentages
            'fg_pct', 'fg2_pct', 'fg3_pct', 'ft_pct', 'fg3_rate', 'ts_pct',
            ...
            # Four Factors - Offense
            'efg_pct', 'orb_pct', 'tov_pct', 'ftr',
            
            # Four Factors - Defense
            'opp_efg_pct', 'opp_orb_pct', 'opp_tov_pct', 'opp_ftr',
            ...
            # Defense metrics
            'stl_pct', 'blk_pct', 'stl_to_ratio', 'stocks_per_100',
            
            # Foul metrics
            'pf_per_100', 'stl_per_pf', 'blk_per_pf',
        ]
...
class TeamSeasonMetricsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'team_name', 'team_slug', 'season_year',
            'games', 'ppg', 'papg', 'pace',
            'ortg', 'drtg', 'net_rtg',
            'efg_pct', 'tov_pct', 'orb_pct', 'ftr',
            'opp_efg_pct', 'opp_tov_pct', 'drb_pct', 'opp_ftr',
            'efg_margin', 'tov_edge', 'reb_edge', 'ftr_margin',
            'kill_shots_for', 'kill_shots_against',
            'kill_shots_pg', 'kill_shots_conceded_pg', 'kill_shot_margin_pg',
            'ast_g', 'ast_pct', 'blk_g', 'blk_pct', 'dpf_g',
            'last_updated',
        ]
...
class TeamSeasonRatingsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'adj_efg_pct', 'adj_tov_pct', 'adj_orb_pct', 'adj_ftr',
            # Adjusted four factors - defense
            'adj_opp_efg_pct', 'adj_opp_tov_pct', 'adj_drb_pct', 'adj_opp_ftr',
            # Adjusted margins
